...
> **Spec folder**: `$SPEC_DIR/` | **Mode**: _(Step 0)_ | **Size**: _(Step 0)_
...
fail ×3, policy violation. (5) Artifacts in `$SPEC_DIR/`. (6) Update
`FEATURE_STATE.json` after each step. (7) Log OpenRouter calls to `cost-log.jsonl`.
(8) **Telemetry**: log step start/end to `$SPEC_DIR/timings.jsonl`
...
Look for `$SPEC_DIR/FEATURE_STATE.json`. If found: display progress, ask user via
...
- [ ] Read `$DOCS_DIR/invariants/features.md`. Identify the feature to build.
...
**Design guide (frontend/fullstack only):** Read `$DOCS_DIR/design/README.md` for the
index, then read the specific `$DOCS_DIR/design/*.md` files relevant to the current
...
timeout 20 python3 -m lib.observability.recall \
  --query "$NAME — $SLUG" --kind decision --repo-root "$REPO_ROOT" --scope repo || true
timeout 20 python3 -m lib.observability.recall \
  --query "$NAME — $SLUG" --kind answer --repo-root "$REPO_ROOT" --scope repo || true
```
...
- [ ] Report saved to `$SPEC_DIR/codebase-scan-light.md`
...
Write `$SPEC_DIR/discovery-output.md`: Mode, Size, Feature (1-line), Context category,
