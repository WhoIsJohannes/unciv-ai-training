...
`$SPEC_DIR/timings.jsonl` (`{"step": N, "phase": 3, "event": "start|end", "ts": "..."}`).
...
Create `$SPEC_DIR/progress.md` with all plan items as checkboxes + Codebase Patterns section.

- [ ] `$SPEC_DIR/progress.md` created
...
- [ ] Read `$SPEC_DIR/progress.md`, pick highest-priority incomplete items
...
a sidecar JSON to `$SPEC_DIR/<gate>-findings.json` (schema_version=1). Exit code
...
if [ -d src/systems ]; then
  SPEC_DIR=$SPEC_DIR python3 "$LIB_DIR/static/src_self_contained.py" --src-root src
fi

# Allowlist override path (per repo): .feature-workflow/design-system-allowlist.txt
SPEC_DIR=$SPEC_DIR python3 "$LIB_DIR/static/visual_gate.py" \
  --repo "$REPO_ROOT" --base "$BASE_BRANCH" --head HEAD --lib-dir "$LIB_DIR"

# Tier A (hidden ARIA seeds) + Tier C (defaultDeps throw-stubs).
SPEC_DIR=$SPEC_DIR python3 "$LIB_DIR/static/behavior_gate.py" \
  --repo "$REPO_ROOT" --base "$BASE_BRANCH" --head HEAD --mode both
```
...
**13e.** Log iteration in `$SPEC_DIR/progress.md`.
...
#### Cleanup-opportunity scan → `$REPO_ROOT/.feature-workflow/cleanup-opportunities.md`
...
`$REPO_ROOT/.feature-workflow/cleanup-opportunities.md` — NOT per-spec. Items
...
2. For each new finding, decide:
   - **Duplicate** of an existing item (same `file:` + `kind:`, or paraphrased text
     ...
     `cleanup_artifact.append_originating_feature(text, existing.uuid, $SLUG)` to
     record provenance.
   - **Refines** an existing item (same target but new detail) → MERGE: update the
     existing item's text/meta in-place; append `$SLUG` to `originating_features:`.
...
with this mission: read `$SPEC_DIR/plan.md`, `$SPEC_DIR/progress.md`, and
`$SPEC_DIR/agent-test-spec.md`; produce a table mapping every plan item to its
implementing commits/files AND to a covering test. Flag rows where either column
is empty. Write the table to `$SPEC_DIR/traceability.md`.
...
Write `$SPEC_DIR/checkpoint.md`: what was built, key files, open issues, gate status,
traceability, test spec path, council plan review path.

- [ ] `$SPEC_DIR/checkpoint.md` written
...
**If a trigger IS present:** write `$SPEC_DIR/retreat.md` and fire a
...
Write `$SPEC_DIR/build-output.md`: summary, files changed, gate status, traceability,
test results, open issues, plan fidelity, security checklist status.

### Sync feature branch with origin/$BASE_BRANCH

Before advancing state, pull in any work that landed on `$BASE_BRANCH` while this
...
```bash
source $LIB_DIR/sync-with-base.sh 3 || exit 1
```
