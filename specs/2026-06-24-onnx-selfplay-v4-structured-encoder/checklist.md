# v4 — Structured Encoder (hex GNN + embeddings + attention) for ONNX self-play — Phase 1: Discovery

Progress tracker for Steps 0–3. Tick boxes and fill evidence after EVERY step.
Save this file to disk after each step.

> **Slug**: `onnx-selfplay-v4-structured-encoder` | **Branch**: `onnx-selfplay-loop` | **Started**: 2026-06-24
> **Spec folder**: `specs/2026-06-24-onnx-selfplay-v4-structured-encoder/` | **Mode**: _(Step 0)_ | **Size**: _(Step 0)_
...
fail ×3, policy violation. (5) Artifacts in `specs/2026-06-24-onnx-selfplay-v4-structured-encoder/`. (6) Update
`FEATURE_STATE.json` after each step. (7) Log OpenRouter calls to `cost-log.jsonl`.
(8) **Telemetry**: log step start/end to `specs/2026-06-24-onnx-selfplay-v4-structured-encoder/timings.jsonl`
(`{"step": N, "phase": 1, "event": "start|end", "ts": "..."}`).
(8b) **Q&A capture**: after every `AskUserQuestion` gate, append one line to
`specs/2026-06-24-onnx-selfplay-v4-structured-encoder/questions.jsonl`: `{"ts", "phase", "step", "header", "question",
...
Look for `specs/2026-06-24-onnx-selfplay-v4-structured-encoder/FEATURE_STATE.json`. If found: display progress, ask user via
...
cat > specs/2026-06-24-onnx-selfplay-v4-structured-encoder/FEATURE_STATE.json << STATEEOF
{ "slug": "onnx-selfplay-v4-structured-encoder", "started": "2026-06-24", "mode": "$MODE", "size": "$SIZE",
  "current_phase": 1, "current_step": 1, "last_completed_step": 0,
  "spec_dir": "specs/2026-06-24-onnx-selfplay-v4-structured-encoder", "repo_root": "/Users/j/Unciv",
...
- [ ] Hooks configured
- [ ] In worktree on branch `onnx-selfplay-v4-structured-encoder`
...
timeout 20 python3 -m lib.observability.recall \
  --query "v4 — Structured Encoder (hex GNN + embeddings + attention) for ONNX self-play — onnx-selfplay-v4-structured-encoder" --kind decision --repo-root "/Users/j/Unciv" --scope repo || true
timeout 20 python3 -m lib.observability.recall \
  --query "v4 — Structured Encoder (hex GNN + embeddings + attention) for ONNX self-play — onnx-selfplay-v4-structured-encoder" --kind answer --repo-root "/Users/j/Unciv" --scope repo || true
```
...
- [ ] Report saved to `specs/2026-06-24-onnx-selfplay-v4-structured-encoder/codebase-scan-light.md`
...
Write `specs/2026-06-24-onnx-selfplay-v4-structured-encoder/discovery-output.md`: Mode, Size, Feature (1-line), Context category,
