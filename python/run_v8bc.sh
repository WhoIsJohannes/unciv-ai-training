#!/usr/bin/env bash
# v8 UNIT-INTENT ablation precursor — does the unit-intent head need the KL-to-clone leash (as construction
# did), or is the tight BC clone alone enough? 3 arms × 4 seeds, paired. If `on` (no leash) already beats
# `off`, the leash is optional; if only `onkl` beats `off`, the leash is load-bearing (as in v7.4). Then scale
# the winner to 8 seeds via run_v8kl.sh.
#
#   TH=6 CONC=2 ./run_v8bc.sh
#
# BASE (all arms): construction ON + BC(120) + construction-KL 0.5 (the v7.4 winning base).
#   off  : --control-unit-intent off  --unit-kl-coef 0
#   on   : --control-unit-intent on   --unit-kl-coef 0      (BC clone, NO unit leash)
#   onkl : --control-unit-intent on   --unit-kl-coef 0.5    (BC clone + unit leash)
set -uo pipefail
cd "$(dirname "$0")"
...
# gradlew lives at the repo root AND gradle must be invoked from there; BC gen runs in a `( cd .. )` subshell.
ROOT="${OUT_ROOT:-../training-runs/v8bc}"; mkdir -p "$ROOT"; ROOT="$(cd "$ROOT" && pwd)"
SEEDS="${SEEDS:-1000 2000 3000 4000}"; TH="${TH:-6}"; CONC="${CONC:-2}"
...
run_job() {  # <arm> <seed> <unit_ctrl> <unit_kl>
  ...
  log "RUN ${arm}_s${seed} (unit_control=$uctrl unit_kl=$ukl)"
...
JOBS=()
for s in $SEEDS; do JOBS+=("off|$s|off|0" "on|$s|on|0" "onkl|$s|on|$UKL"); done
for job in "${JOBS[@]}"; do
...
python3 analyze_v8.py "$ROOT" "$SEEDS" "off,on,onkl" "on-off;onkl-off;onkl-on"
